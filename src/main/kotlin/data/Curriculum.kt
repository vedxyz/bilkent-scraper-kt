...
internal fun parseCurriculum(dom: Document) = dom.select("table.printMod").drop(2).dropLast(2).map { table ->
    table.select("tbody > tr").drop(1).map { row ->
        ...
        val semesterMatcher = row.child(5).text().trim().let { semesterPattern.matcher(it) }
        val replacementMatcher = row.child(6).text().trim().let { replacementPattern.matcher(it) }
        ...
        CurriculumCourseItem(course = if (course.size != 2) null else Course(course[0], course[1]),
            ...
            semester = if (semesterMatcher.find()) semesterMatcher.run {
                Semester(
                    year = group(1), season = SemesterType.valueOf(group(2))
                )
            } else null,
            replacement = if (replacementMatcher.find()) replacementMatcher.run {
                CurriculumCourseReplacement(
                    Course(
...
internal suspend fun getCurriculum(cookie: String): List<CurriculumSemester> {
    val responseText = OkHttpSingleton.guardedFetch("https://stars.bilkent.edu.tr/srs/ajax/curriculum.php", cookie)
